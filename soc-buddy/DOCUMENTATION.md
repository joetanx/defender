# SOC Buddy Technical Architecture & Identity Documentation

This document provides a comprehensive technical overview of **SOC Buddy**, an AI teammate built for Security Operations Center (SOC) teams. SOC Buddy integrates **Microsoft Teams**, **Azure Container Apps**, **Microsoft Agent 365**, **Azure AI Foundry**, **Microsoft Sentinel**, **Defender XDR**, and **Microsoft Graph Security**.

---

## Table of Contents
1. [Architecture Overview](#1-architecture-overview)
   - [Component Topology](#11-component-topology)
   - [Component Descriptions & Interactions](#12-component-descriptions--interactions)
2. [Identity Architecture & Token Flows](#2-identity-architecture--token-flows)
   - [Identity Models Overview](#21-identity-models-overview)
   - [Teams Bot Identity](#22-teams-bot-identity)
     - [Bot Framework Ingress & JWT Validation](#221-bot-framework-ingress--jwt-validation)
     - [OAuth Authorization Code Flow for Human Assertion](#222-oauth-authorization-code-flow-for-human-assertion)
   - [Agent Identity](#23-agent-identity)
     - [Agent Blueprint & Federated Identity Credentials (FIC)](#231-agent-blueprint--federated-identity-credentials-fic)
     - [Agent 365 Observability via OpenTelemetry](#232-agent-365-observability-via-opentelemetry)
     - [On-Behalf-Of (OBO) Delegation with User Assertion](#233-on-behalf-of-obo-delegation-with-user-assertion)
   - [End-to-End Sequence Diagram](#24-end-to-end-sequence-diagram)
3. [Tool Catalog & Delegated Permissions](#3-tool-catalog--delegated-permissions)
   - [Permission Summary Matrix](#31-permission-summary-matrix)
   - [Microsoft Sentinel MCP Data Exploration Tools](#32-microsoft-sentinel-mcp-data-exploration-tools)
   - [Microsoft Sentinel MCP Defender Triage Tools](#33-microsoft-sentinel-mcp-defender-triage-tools)
   - [Work IQ Mail MCP Tools](#34-work-iq-mail-mcp-tools)
   - [Microsoft Graph Security Incident Tools](#35-microsoft-graph-security-incident-tools)
   - [Built-In Utility Tools](#36-built-in-utility-tools)
...
SOC Buddy operates as a containerized Python service deployed in **Azure Container Apps** that bridges human security analysts in Microsoft Teams with LLM reasoning (Azure AI Foundry) and security tools (Model Context Protocol servers and Microsoft Graph).

```mermaid
graph TB
    subgraph Client ["Security Analyst Workplace"]
        Analyst[Security Analyst]
        Teams[Microsoft Teams Desktop / Web]
        Analyst -->|1. Chat & Prompts| Teams
    end

    subgraph Channel ["Messaging & Gateway"]
        AB[Azure Bot Service / Bot Framework]
        Teams -->|2. Encrypted Bot Traffic| AB
    end

    subgraph Host ["Azure Container Apps (SOC Buddy Host)"]
        CA[Container App: aiohttp web server]
        Adapter[CloudAdapter / JWT Validator]
        AgentEngine[LangChain Agent + State Trimmer]
        MSAL[MSAL Client & Token Cache]
        OtelExp[Microsoft OpenTelemetry Exporter]

        AB -->|3. POST /api/messages| CA
        CA --> Adapter
        Adapter --> AgentEngine
        AgentEngine <--> MSAL
        AgentEngine -.->|Export traces| OtelExp
    end

    subgraph ModelHost ["Inference"]
        Foundry[Azure AI Foundry / OpenAI Model]
        AgentEngine -->|4. Chat Completions| Foundry
    end

    subgraph Entra ["Microsoft Entra ID"]
        TokenEndpoint[OAuth2 / v2.0 Token Endpoint]
        MSAL -->|5. Assertion & OBO Exchange| TokenEndpoint
    end

    subgraph Downstream ["Downstream MCP & Graph APIs"]
        MCP_Explore[Sentinel MCP Data Exploration]
        MCP_Triage[Sentinel MCP Defender Triage]
        MCP_Mail[Work IQ Mail MCP]
        GraphSec[Microsoft Graph Security API]

        AgentEngine -->|6a. KQL & Hunting| MCP_Explore
        AgentEngine -->|6b. Alerts & Incident Triage| MCP_Triage
        AgentEngine -->|6c. Email Communication| MCP_Mail
        AgentEngine -->|6d. Comments & Status Patches| GraphSec
    end

    subgraph Observability ["Telemetry"]
        A365Obs[Agent 365 Observability Service]
        OtelExp -->|7. S2S Telemetry Export| A365Obs
    end
```

### 1.2. Component Descriptions & Interactions

1. **Microsoft Teams Client**:
   The interactive interface for the SOC analyst. Analysts query incidents, request KQL hunting queries, draft investigation summaries, and receive Adaptive Cards for authentication.
2. **Azure Bot Service / Teams Bot Framework**:
   Routes messages between Microsoft Teams and the containerized endpoint (`/api/messages`). Uses HMAC and JWT tokens signed by Microsoft Bot Framework roots to authenticate payloads.
3. **Azure Container App (SOC Buddy Service)**:
   - Hosted on Linux within an Azure Container Apps Environment.
   - Built on `aiohttp` running the **Microsoft Agents SDK** and **LangChain**.
   - Integrates `MultiServerMCPClient` to connect dynamically to remote Model Context Protocol (MCP) endpoints.
4. **Azure AI Foundry (Cognitive Services)**:
   Provides foundational LLM capabilities (e.g., `gpt-4o`). The Container App authenticates directly to Foundry using Azure **User-Assigned Managed Identity (UAMI)**, requiring no API keys.
5. **Microsoft Agent 365 Runtime**:
   Manages the lifecycle, registration, and governance of the agent as an organizational AI teammate. Provides built-in telemetry baggage propagation and audit logging.
6. **Remote MCP Servers**:
   Standardized tool providers running over Streamable HTTP transports. Each MCP server requires bearer token authentication issued by Microsoft Entra ID.
7. **Microsoft Graph Security API**:
   Native REST interface for reading, commenting on, assigning, and closing security incidents across Microsoft Defender XDR and Sentinel.

---

## 2. Identity Architecture & Token Flows
...
### 2.1. Identity Models Overview

There are three distinct identities involved in SOC Buddy:

```
+-----------------------------------+-----------------------------------+-----------------------------------+
| User-Assigned Managed Identity    | Teams Bot Application             | Agent Blueprint / Identity        |
| (UAMI)                            | (Client App)                      | (Agent 365 AI Teammate)           |
+-----------------------------------+-----------------------------------+-----------------------------------+
| - Azure Resource identity         | - Entra App Registration          | - Entra App & Service Principal   |
| - Pulls images from ACR           | - Receives messages from Teams    | - Represents the AI Teammate      |
| - Authenticates to AI Foundry     | - Prompts analyst for sign-in     | - Possesses delegated scopes      |
| - Acts as federated credential    | - Federates with UAMI via FIC     | - Federates with UAMI via FIC     |
|   subject for Entra apps          | - Caches analyst user assertion   | - Performs OBO token exchanges    |
+-----------------------------------+-----------------------------------+-----------------------------------+
```

---

### 2.2. Teams Bot Identity

#### 2.2.1. Bot Framework Ingress & JWT Validation
...
#### 2.2.2. OAuth Authorization Code Flow for Human Assertion
...
#### 2.3.1. Agent Blueprint & Federated Identity Credentials (FIC)
...
#### 2.3.2. Agent 365 Observability via OpenTelemetry
...
#### 2.3.3. On-Behalf-Of (OBO) Delegation with User Assertion
...
| Resource API | Application ID / Client ID | Scope / Role Name | Scope ID (GUID) | Type | Purpose |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Sentinel Data Exploration MCP** | `4500ebfb-89b6-4b14-a480-7f749797bfcd` | `SentinelPlatform.DelegatedAccess` | `eaff9684-612c-4add-aa10-035fd3bfe3d1` | Delegated (Scope) | Execute KQL hunting queries, discover workspace tables, read log telemetry. |
| **Sentinel Defender Triage MCP** | `7b7b3966-1961-47b5-b080-43ca5482e21c` | `MCP.Read.All` | `8dd500d0-c3aa-4380-96d1-09b4b6233eff` | Delegated (Scope) | Inspect Defender XDR incidents, alert evidence, impacted devices, and identities. |
| **Work IQ Mail MCP** | `16b1878d-62c7-4009-aa25-68989d63bbad` | `Tools.ListInvoke.All` | `93aac09f-5f9b-4b4c-aa45-c623a1b69342` | Delegated (Scope) | Read analyst email notifications, search incident email threads, draft communications. |
| **Microsoft Graph** | `00000003-0000-0000-c000-000000000000` | `SecurityIncident.ReadWrite.All` | `aaad2076-26ab-4905-b1eb-090f627b17d7` | Delegated (Scope) | Post comments to incidents and update status, classification, determination, and tags. |
| **Agent 365 Observability** | `9b975845-388f-4429-889e-eab1ef63949c` | `api://9b975845-388f-4429-889e-eab1ef63949c/.default` | N/A | Application (S2S) | Export agent spans, traces, and metrics to Microsoft Agent 365 control plane. |

---

### 3.2. Microsoft Sentinel MCP Data Exploration Tools
...
### 3.3. Microsoft Sentinel MCP Defender Triage Tools
...
### 3.5. Microsoft Graph Security Incident Tools
